...
import { Heart, Church, Save, MapPin, User, X, MessageCircle, ArrowLeft, LogOut, Edit, Check, CheckCheck, AlertTriangle, Eye, EyeOff, SlidersHorizontal, Lock } from 'lucide-react'
...
function App() {
  ...
  const [view, setView] = useState('discovery') // 'discovery', 'matches', 'stats', 'chat', 'setup', 'profile', 'blocked', 'security'
  ...
  useEffect(() => {
    ...
    if (savedDistance) setFilterDistance(filterDistance) 
  }, [])
  ...
  const handlePasswordResetSubmit = async (e) => {
    e.preventDefault()
    if (newResetPass.length < 6) { alert("Password must be at least 6 characters."); return }
    if (newResetPass !== confirmResetPass) { alert("Passwords do not match."); return }
    setLoading(true)
    try {
      const { error } = await supabase.auth.updateUser({ password: newResetPass })
  ...
  const handleResendEmail = async () => {
    ...
    if (error) { alert(error.message) } else { alert("Verification email sent again!") }
  }
  ...
  const handleForgotPassword = async (e) => {
    e.preventDefault()
    if (!resetEmail || !resetEmail.includes('@')) { alert("Please enter a valid email."); return }
    setLoading(true)
    try {
  ...
  useEffect(() => {
      ...
      .on('presence', { event: 'sync' }, () => {
        ...
        Object.values(state).forEach(presences => {
          if (Array.isArray(presences)) {
            presences.forEach(p => { if (p.user_id) userIds.add(p.user_id) })
    ...
    return () => { if (globalPresenceChannelRef.current) supabase.removeChannel(globalPresenceChannelRef.current) }
  }, [session])
  ...
  async function fetchProfile(userId) {
    try {
      ...
      if (profileError) {
        if (profileError.code === 'PGRST116') { supabase.auth.signOut(); return }
        throw profileError
      }
      ...
      if (!myProfile.gender || !myProfile.intent) setView('setup')
      else await fetchCandidates(userId, myProfile.gender, myProfile)
    } catch (error) {
  ...
  async function fetchCandidates(myId, myGender, myCurrentProfile) {
    ...
    const { data: existingMatches } = await supabase.from('matches').select('user_a_id, user_b_id').or(`user_a_id.eq.${myId},user_b_id.eq.${myId}`)
    const matchedIds = existingMatches ? existingMatches.map(m => m.user_a_id === myId ? m.user_b_id : m.user_a_id) : []
    
    const { data: blocked } = await supabase.from('blocks').select('blocked_id').eq('blocker_id', myId)
    ...
    let query = supabase.from('profiles').select('*').eq('gender', targetGender).neq('id', myId)
    if (excludeIds.length > 0) query = query.not('id', 'in', `(${excludeIds.join(',')})`)

    if (filterCity) query = query.eq('city', filterCity)
    else if (filterDistance) { /* JS Filter later */ }
    else if (!hasLocation) query = query.eq('city', myCurrentProfile?.city)
    ...
    if (hasLocation) {
        ...
        if (error) console.error('Error fetching candidates:', error)
            ...
            const candidatesWithDistance = profiles
              ...
              .map(p => ({ ...p, distance: calculateDistance(myCurrentProfile.lat, myCurrentProfile.long, p.lat, p.long) }))
              .sort((a, b) => a.distance - b.distance)
              
            let finalCandidates = candidatesWithDistance
            if (filterDistance) finalCandidates = candidatesWithDistance.filter(p => p.distance <= parseInt(filterDistance))
  ...
  const fetchBlockedUsers = async () => {
    if (!session) return
    try {
      const { data: blocks, error: blockError } = await supabase.from('blocks').select('id, blocked_id').eq('blocker_id', session.user.id)
      ...
      const merged = blocks.map(block => { const profile = profiles.find(p => p.id === block.blocked_id); return { ...block, profile } })
  ...
  const fetchMyMatches = async () => {
    if (!session) { setMyMatches([]); setPartnerProfiles([]); setUnreadCounts({}); return }
    try {
      const { data: matches, error: matchError } = await supabase.from('matches').select('*').or(`user_a_id.eq.${session.user.id},user_b_id.eq.${session.user.id}`).or('status.eq.mutual,status.eq.pending')
      if (matchError) { setMyMatches([]); setPartnerProfiles([]); setUnreadCounts({}); return }
      if (!matches || matches.length === 0) { setMyMatches([]); setPartnerProfiles([]); setUnreadCounts({}); return }
      
      const partnerIds = matches.map((m) => m.user_a_id === session.user.id ? m.user_b_id : m.user_a_id)
      const uniquePartnerIds = [...new Set(partnerIds)]
      const { data: profiles, error: profileError } = await supabase.from('profiles').select('*').in('id', uniquePartnerIds)
      if (profileError) { setMyMatches(matches); setPartnerProfiles([]); setUnreadCounts({}); return }
      ...
      if (matches && matches.length > 0) {
          const countPromises = matches.map(async (match) => {
              ...
              const { count } = await supabase.from('messages').select('*', { count: 'exact', head: true }).eq('match_id', match.id).neq('sender_id', session.user.id).is('read_at', null)
              return { partnerId, count }
          })
          const results = await Promise.all(countPromises)
          results.forEach(({ partnerId, count }) => { if (count > 0 && partnerId !== lastOpenedChatId) counts[partnerId] = count })
      }
      setUnreadCounts(counts)
    } catch (error) { console.error("Error:", error) }
  }

  useEffect(() => { if (view === 'matches' && session) fetchMyMatches() }, [view, lastOpenedChatId])
  ...
  const handleAuth = async (e) => {
    ...
    if (authMode === 'login') {
      ...
      const { error } = await supabase.auth.signUp({ email, password, options: { data: { full_name: 'New User' } } })
      if (error) {
        alert(error.message)
      } else {
  ...
  useEffect(() => { if (view === 'setup' || view === 'profile') getUserLocation() }, [view])

  const handlePasswordUpdate = async (e) => {
    e.preventDefault()
    if (!oldPassword) { alert("Enter current password."); return }
    if (newPassword.length < 6) { alert("New password must be 6+ chars."); return }
  ...
  const handleSaveProfile = async (e) => {
    e.preventDefault()
    if (!session) return
    if (!dateOfBirth) { alert("Enter Date of Birth."); return }
    ...
    const { error } = await supabase.from('profiles').update({
        ...
        date_of_birth: dateOfBirth, avatar_url: finalAvatarUrl,
        lat: userCoords.lat, long: userCoords.long, updated_at: new Date(),
    }).eq('id', session.user.id)
    if (error) alert(error.message)
    else {
      alert(view === 'setup' ? 'Profile Saved!' : 'Updated!')
  ...
  const handleConnect = async () => {
    ...
    setLoading(true)
    const { data: matchesData, error: checkError } = await supabase.from('matches').select('*').eq('user_a_id', targetUser.id).eq('user_b_id', session.user.id).limit(1)
    ...
    if (existingMatch) {
      ...
      await supabase.from('matches').insert({ user_a_id: session.user.id, user_b_id: targetUser.id, status: 'mutual' })
      showNotification("🎉 It's a Match!", `You matched with ${targetUser.full_name}.`)
    } else {
      alert("Connection Requested! 💌")
      const { error } = await supabase.from('matches').insert({ user_a_id: session.user.id, user_b_id: targetUser.id, status: 'pending' })
  ...
  const handleUnmatch = async (partnerId) => {
    if (!window.confirm("Unmatch?")) return
    try {
      const match = myMatches.find(m => (m.user_a_id === session.user.id && m.user_b_id === partnerId) || (m.user_b_id === session.user.id && m.user_a_id === partnerId))
      if (!match) return
      ...
      alert("Unmatched.")
    } catch (err) { alert("Could not unmatch.") }
  }

  const handleAcceptRequest = async (matchId, partnerId) => {
    try {
      await supabase.from('matches').update({ status: 'mutual' }).eq('id', matchId)
      await supabase.from('matches').insert({ user_a_id: session.user.id, user_b_id: partnerId, status: 'mutual' })
      fetchMyMatches()
      alert("🎉 Matched!")
    } catch (err) { alert("Error accepting match.") }
  }

  const handleRejectRequest = async (matchId) => {
    if (!window.confirm("Decline request?")) return
    try { await supabase.from('matches').delete().eq('id', matchId); fetchMyMatches() } catch (err) {}
  }

  const handleReportUser = async (reportedId) => {
    const reason = window.prompt("Why report this user?")
    if (!reason) return
    try {
      await supabase.from('reports').insert({ reporter_id: session.user.id, reported_id: reportedId, reason: reason, message: "Reported via chat" })
      alert("Report submitted.")
    } catch (err) { alert("Could not submit report.") }
  }
  ...
  const handleBlock = async (partnerId) => {
    ...
    try {
      ...
      if (!match) return
      await supabase.from('matches').delete().eq('id', match.id)
      await supabase.from('blocks').insert({ blocker_id: session.user.id, blocked_id: partnerId })
      setPartnerProfiles(prev => prev.filter(p => p.id !== partnerId))
      setUnreadCounts(prev => { const copy = { ...prev }; delete copy[partnerId]; return copy })
      setMyMatches(prev => prev.filter(m => m.id !== match.id))
      alert("Blocked.")
    } catch (err) { alert("Could not block.") }
  }
  ...
  const handleInputChange = (e) => {
    ...
    if (text.trim() === "") {
        typingChannelRef.current?.send({ type: 'broadcast', event: 'stop_typing', payload: { userId: session.user.id, typing: false } })
        if (typingTimeout.current) clearTimeout(typingTimeout.current)
    } else {
        if (typingTimeout.current) clearTimeout(typingTimeout.current)
        typingTimeout.current = setTimeout(() => {
            if (view === 'chat' && activeChatProfile && text.length > 0) {
                 typingChannelRef.current?.send({ type: 'broadcast', event: 'typing', payload: { userId: session.user.id, typing: true } })
            }
        }, 500)
    }
  }
  ...
  const sendMessage = async () => {
    ...
    const match = myMatches.find(m => (m.user_a_id === session.user.id && m.user_b_id === activeChatProfile.id) || (m.user_b_id === session.user.id && m.user_a_id === activeChatProfile.id))
    if (!match) return
    typingChannelRef.current?.send({ type: 'broadcast', event: 'stop_typing', payload: { userId: session.user.id, typing: false } })
    if (typingTimeout.current) clearTimeout(typingTimeout.current)
    const { error } = await supabase.from('messages').insert({ match_id: match.id, sender_id: session.user.id, content: inputText, read_at: null })
    if (error) console.error("Error:", error)
    else { setInputText(""); if (chatInputRef.current) chatInputRef.current.style.height = 'auto'; }
  }
  ...
  const openChat = async (profile) => {
    ...
    setIsPartnerOnline(onlineUsers.includes(profile.id))

    const match = myMatches.find(m => (m.user_a_id === session.user.id && m.user_b_id === profile.id) || (m.user_b_id === session.user.id && m.user_a_id === profile.id))
    if (!match) return
    const currentMatchId = match.id

    setLastOpenedChatId(profile.id)
    setUnreadCounts(prev => { const copy = { ...prev }; delete copy[profile.id]; return copy })
    await fetchMessages(currentMatchId)

    requestAnimationFrame(() => { if (messagesEndRef.current) messagesEndRef.current.scrollIntoView({ behavior: 'smooth' }) })

    await supabase.from('messages').update({ read_at: new Date() }).eq('match_id', currentMatchId).neq('sender_id', session.user.id).is('read_at', null)
    setUnreadCounts(prev => ({ ...prev, [profile.id]: 0 }))

    if (realtimeChannel) supabase.removeChannel(realtimeChannel)

    const messageChannel = supabase.channel(`messages:${currentMatchId}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `match_id=eq.${currentMatchId}` }, payload => {
          setChatMessages(prev => {
            ...
            if (payload.new.sender_id === session.user.id) {
                ...
                if (existingIndex > -1) newPrev[existingIndex] = { ...payload.new, is_delivered: true }
                else newPrev.push({ ...payload.new, is_delivered: true })
            } else newPrev.push(payload.new)
            return newPrev
          })
          const isMe = payload.new.sender_id === session.user.id
          if (!isMe && !document.hasFocus()) showNotification(`New message`, payload.new.content)
      })
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'messages', filter: `match_id=eq.${currentMatchId}` }, payload => {
          setChatMessages(prev => prev.map(msg => msg.id === payload.new.id ? payload.new : msg))
      })
      .subscribe()
    messageChannelRef.current = messageChannel

    const typingChannel = supabase.channel(`typing:${currentMatchId}`, { config: { broadcast: { self: false } } })
      .on('broadcast', { event: 'typing' }, () => { setPartnerIsTyping(true); if (partnerTypingTimeout.current) clearTimeout(partnerTypingTimeout.current); partnerTypingTimeout.current = setTimeout(() => setPartnerIsTyping(false), 3000) })
      .on('broadcast', { event: 'stop_typing' }, () => { setPartnerIsTyping(false); if (partnerTypingTimeout.current) clearTimeout(partnerTypingTimeout.current) })
      .subscribe()
    typingChannelRef.current = typingChannel

    const presenceChannel = supabase.channel(`presence:${currentMatchId}`, { config: { presence: { key: session.user.id } } })
      .on('presence', { event: 'sync' }, () => { const state = presenceChannel.presenceState(); setIsPartnerOnline(!!state[profile.id]) })
      .subscribe()
    presenceChannelRef.current = presenceChannel
    setRealtimeChannel(typingChannel)
  }

  useEffect(() => { if (view === 'chat' && chatMessages.length > 0) requestAnimationFrame(() => { if (messagesEndRef.current) messagesEndRef.current.scrollIntoView({ behavior: 'smooth' }) }) }, [chatMessages])

  useEffect(() => {
    if (view !== 'chat') {
      if (typingChannelRef.current) supabase.removeChannel(typingChannelRef.current)
      if (messageChannelRef.current) supabase.removeChannel(messageChannelRef.current)
      typingChannelRef.current = null; messageChannelRef.current = null
    }
  }, [view])
      ...
      return (
        <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
            <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full text-center">
                <div className="inline-flex items-center justify-center w-16 h-16 bg-green-100 rounded-full mb-4"><Lock size={32} className="text-green-600" /></div>
                <h2 className="text-2xl font-bold text-gray-900 mb-2">Set New Password</h2>
                <form onSubmit={handlePasswordResetSubmit} className="space-y-4 mt-6">
                    <input type="password" required placeholder="New password" className="w-full p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-rose-500 outline-none" value={newResetPass} onChange={(e) => setNewResetPass(e.target.value)} />
                    <input type="password" required placeholder="Confirm password" className="w-full p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-rose-500 outline-none" value={confirmResetPass} onChange={(e) => setConfirmResetPass(e.target.value)} />
                ...
                <button onClick={handleBackToLogin} className="mt-4 text-sm text-gray-500 hover:text-gray-800">Back to Login</button>
            </div>
        </div>
      )
  }
  ...
  if (isSignupSuccess) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white p-10 rounded-3xl shadow-2xl text-center animate-fade-in-up max-w-md">
          <div className="inline-flex items-center justify-center w-20 h-20 bg-green-100 rounded-full mb-6"><svg className="w-10 h-10 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7"/></svg></div>
          <h1 className="text-3xl font-bold mb-2">Welcome!</h1>
          <p className="text-gray-600 mb-8">Account verified.</p>
          <button onClick={() => setIsSignupSuccess(false)} className="w-full bg-rose-600 text-white font-bold py-3 rounded-xl">Continue</button>
        </div>
      </div>
    )
  }
  ...
  if (!session) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4" style={loginWallpaperStyle}>
        <div className="bg-white/90 backdrop-blur-sm p-8 rounded-2xl shadow-xl max-w-md w-full text-center">
          <div className="flex justify-center mb-6">
             <svg width="60" height="60" viewBox="0 0 24 24" fill="none" stroke="#e11d48" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
          </div>
          <h1 className="text-3xl font-serif-custom font-bold mb-1">SacredHearts <sup className="text-rose-400 text-sm align-top">GH</sup></h1>
          <p className="text-gray-500 mb-8 text-sm">Connecting Hearts Under Grace</p>
          
          {authMode === 'signup' && (
            <div className="bg-blue-50 border border-blue-200 p-3 rounded-lg mb-4 text-xs text-blue-800 text-center">
                💡 Check your Spam folder if you don't see the email.
            </div>
          )}

          <form onSubmit={handleAuth} className="space-y-4 text-left">
            <input type="email" placeholder="Email" required className="w-full p-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-rose-500 outline-none" value={email} onChange={e => setEmail(e.target.value)} />
            <div className="relative">
                <input type={showPassword ? "text" : "password"} placeholder="Password" required className="w-full p-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-rose-500 outline-none" value={password} onChange={e => setPassword(e.target.value)} />
                ...
                <button type="button" onClick={() => { setResetEmail(email); setShowForgotModal(true); }} className="absolute -bottom-6 right-0 text-xs text-rose-600 font-semibold">Forgot password?</button>
            </div>
            <button type="submit" disabled={loading} className="w-full bg-rose-600 text-white py-3 rounded-xl font-bold shadow-lg hover:bg-rose-700 transition">{authMode === 'login' ? 'Log In' : 'Sign Up'}</button>
          </form>

          <div className="mt-10">
             <button onClick={() => setAuthMode(authMode === 'login' ? 'signup' : 'login')} className="text-rose-600 font-bold text-sm">{authMode === 'login' ? "Create Account" : "Back to Login"}</button>
          </div>

          {showForgotModal && (
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                  <div className="bg-white p-6 rounded-2xl shadow-2xl w-full max-w-sm animate-fade-in-up relative">
                      <button onClick={() => setShowForgotModal(false)} className="absolute top-4 right-4 text-gray-400"><X size={20}/></button>
                      <h2 className="text-xl font-bold mb-4">Reset Password</h2>
                      <form onSubmit={handleForgotPassword} className="space-y-4">
                          <input type="email" required value={resetEmail} onChange={e => setResetEmail(e.target.value)} className="w-full p-3 border rounded-xl" placeholder="Email" />
                          <button type="submit" disabled={loading} className="w-full bg-rose-600 text-white font-bold py-3 rounded-xl">Send Link</button>
                      </form>
                  </div>
              </div>
          )}
  ...
  if (view === 'setup' || view === 'profile') {
    ...
    return (
      <div className="min-h-screen bg-gray-50 pb-20">
        <div className="max-w-lg mx-auto bg-white p-6 rounded-xl shadow-lg mt-4 mx-4">
          <div className="flex items-center gap-4 mb-6">
            {view === 'profile' && <button onClick={() => setView('discovery')} className="text-gray-600"><ArrowLeft size={24}/></button>}
            <h2 className="text-2xl font-bold">{isEditMode ? 'Edit Profile' : 'Setup Profile'}</h2>
          </div>
          <form onSubmit={handleSaveProfile} className="space-y-4">
            <div className="flex flex-col items-center mb-6">
                <div onClick={() => document.getElementById('avatar-input').click()} className="relative w-28 h-28 rounded-full border-4 border-rose-100 shadow-md overflow-hidden bg-gray-100">
                    <img src={previewUrl || profile?.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${profile?.full_name}&backgroundColor=b6e3f4`} className="w-full h-full object-cover" />
                    {uploading && <div className="absolute inset-0 bg-black/50 flex items-center justify-center text-white text-xs">Uploading...</div>}
                </div>
                <input type="file" id="avatar-input" accept="image/*" onChange={handleFileChange} className="hidden" />
            </div>
            <input type="text" placeholder="Full Name" required value={fullName} onChange={e => setFullName(e.target.value)} className="w-full p-3 border rounded-xl focus:ring-2 focus:ring-rose-500 outline-none" />
            {isEditMode ? <div className="w-full p-3 bg-gray-100 rounded-xl text-gray-500">{gender === 'male' ? 'Man' : 'Woman'}</div> : <select required value={gender} onChange={e => setGender(e.target.value)} className="w-full p-3 border rounded-xl"><option value="">Select Gender</option><option value="male">Man</option><option value="female">Woman</option></select>}
            <div>
                <input type="date" required value={dateOfBirth} onChange={e => setDateOfBirth(e.target.value)} className="w-full p-3 border rounded-xl" max={maxDate} />
                {dateOfBirth && calculateAge(dateOfBirth) < 18 && <p className="text-red-500 text-xs mt-1">Must be 18+</p>}
            </div>
            <select required value={city} onChange={e => setCity(e.target.value)} className="w-full p-3 border rounded-xl"><option value="">City</option><option value="Accra">Accra</option><option value="Kumasi">Kumasi</option><option value="Tema">Tema</option><option value="Tamale">Tamale</option><option value="Cape Coast">Cape Coast</option><option value="Takoradi">Takoradi</option><option value="Sunyani">Sunyani</option><option value="Ho">Ho</option><option value="Wa">Wa</option><option value="Techiman">Techiman</option><option value="Goaso">Goaso</option><option value="Nalerigu">Nalerigu</option><option value="Sefwi Wiaso">Sefwi Wiaso</option><option value="Damango">Damango</option><option value="Dambai">Dambai</option><option value="Bolgatanga">Bolgatanga</option></select>
            <select required value={religion} onChange={e => setReligion(e.target.value)} className="w-full p-3 border rounded-xl"><option value="">Religion</option><option value="Christian">Christian</option><option value="Muslim">Muslim</option><option value="Others">Others</option></select>
            <select required value={intent} onChange={e => setIntent(e.target.value)} className="w-full p-3 border rounded-xl"><option value="">Goal</option><option value="Serious Dating">Serious Dating</option><option value="Marriage">Marriage</option></select>
            <textarea rows="3" placeholder="Bio..." required value={bio} onChange={e => setBio(e.target.value)} className="w-full p-3 border rounded-xl"></textarea>
            <button type="submit" disabled={loading} className="w-full bg-rose-600 text-white font-bold py-3 rounded-xl shadow-lg">{loading ? 'Saving...' : (isEditMode ? 'Update' : 'Save')}</button>    
            
            <button type="button" onClick={() => setView('security')} className="w-full bg-gray-800 text-white font-bold py-3 rounded-xl mt-2 flex justify-center items-center gap-2"><Lock size={18}/> Change Password</button>
            <button type="button" onClick={() => { fetchBlockedUsers(); setView('blocked'); }} className="w-full bg-gray-100 text-gray-700 font-bold py-3 rounded-xl flex justify-center items-center gap-2 mt-2"><AlertTriangle size={18}/> Blocked Users ({blockedUsers.length})</button>
            {isEditMode && <button type="button" onClick={() => supabase.auth.signOut()} className="w-full bg-red-50 text-red-600 font-bold py-3 rounded-xl mt-2 flex justify-center items-center gap-2"><LogOut size={18}/> Logout</button>}
          </form>
        </div>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      {/* --- HEADER --- */}
      <header className="bg-white shadow-sm p-3 flex justify-between items-center sticky top-0 z-20 gap-2">
        {/* Logo Area */}
        ...
        {view === 'discovery' && (
          <div className="w-full max-w-md mx-auto h-full flex flex-col">
            ...
            <div className="flex justify-between items-center mb-4 px-1 z-10">
                ...
                <button onClick={() => setShowFilters(true)} className={`flex items-center gap-2 px-3 py-1.5 rounded-full border text-xs font-medium transition ${(filterCity || filterReligion) ? 'bg-rose-50 border-rose-500 text-rose-600' : 'bg-white border-gray-300 text-gray-600'}`}>
                    <SlidersHorizontal size={14} /> Filters {(filterCity || filterReligion) && <span className="bg-rose-500 text-white text-[9px] px-1 rounded-full">!</span>}
                </button>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-lg">
                <form onSubmit={handlePasswordUpdate} className="space-y-4">
                    <div className="relative">
                        ...
                        <input type={showOldPassword ? "text" : "password"} required value={oldPassword} onChange={e => setOldPassword(e.target.value)} className="w-full p-3 border rounded-xl focus:ring-2 focus:ring-rose-500 outline-none" />
                        <button type="button" onClick={() => setShowOldPassword(!showOldPassword)} className="absolute right-3 top-8 text-gray-400">{showOldPassword ? <EyeOff size={18}/> : <Eye size={18}/>}</button>
                    </div>
                    <div className="relative">
                        <label className="block text-xs font-bold text-gray-500 mb-1 uppercase">New</label>
                        <input type={showNewPassword ? "text" : "password"} required value={newPassword} onChange={e => setNewPassword(e.target.value)} className="w-full p-3 border rounded-xl focus:ring-2 focus:ring-rose-500 outline-none" />
                        <button type="button" onClick={() => setShowNewPassword(!showNewPassword)} className="absolute right-3 top-8 text-gray-400">{showNewPassword ? <EyeOff size={18}/> : <Eye size={18}/>}</button>
                    </div>
                    <button type="submit" disabled={loading} className="w-full bg-rose-600 text-white font-bold py-3 rounded-xl shadow-lg">Update Password</button>
                </form>
            </div>
          ...
          <div className="flex flex-col h-full w-full bg-white sm:rounded-xl sm:shadow-2xl sm:h-[calc(100vh-120px)] sm:max-w-md sm:mx-auto overflow-hidden">
            ...
            <div className="bg-rose-600 text-white p-3 sm:p-4 flex items-center justify-between shrink-0 z-10 shadow-md">
              <div className="flex items-center gap-3 flex-grow min-w-0">
                <button onClick={() => { setView('matches'); if(realtimeChannel) supabase.removeChannel(realtimeChannel); if(typingChannelRef.current) supabase.removeChannel(typingChannelRef.current); setActiveChatProfile(null) }} className="p-1 -ml-1 rounded-full hover:bg-white/10 transition"><ArrowLeft size={24} /></button>
                <div className="relative shrink-0">
                ...
                <div className="min-w-0">
                  <h3 className="font-bold text-base truncate flex items-center gap-2">{activeChatProfile.full_name} {isPartnerOnline && <span className="text-[10px] font-normal text-green-200">Online</span>}</h3>
                  <p className="text-rose-200 text-xs truncate flex items-center gap-1"><MapPin size={10} /> {activeChatProfile.city}</p>
                </div>
              </div>
              <button onClick={() => handleReportUser(activeChatProfile.id)} className="ml-2 shrink-0 bg-white/10 hover:bg-white/20 text-white text-[10px] font-semibold px-2 py-1.5 rounded-full transition"><AlertTriangle size={12} /> Report</button>
            </div>
            ...
            <div className="flex flex-col justify-end shrink-0 bg-white border-t border-gray-200">
              ...
              <div className="chat-input-container">
                <textarea ref={chatInputRef} className="chat-textarea-auto chat-input" value={inputText} onChange={handleInputChange} placeholder="Type a message..." onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage(); } }} />
                <button onClick={sendMessage} className="chat-send-btn bg-rose-600 text-white rounded-full shadow-md hover:bg-rose-700 active:scale-95 transition"><Heart size={18} fill="white" /></button>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50">
          <div className="bg-white w-full max-w-md rounded-t-2xl sm:rounded-2xl p-6 animate-fade-in-up shadow-2xl">
            <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => setShowFilters(false)} className="text-gray-400 hover:text-gray-600"><X size={24}/></button>
            </div>
            <div className="space-y-4">
                ...
                <div className="flex gap-2 mt-6">
                    <button onClick={() => { setFilterCity(''); setFilterReligion(''); setFilterDistance(''); localStorage.removeItem('sacred_distance_filter'); localStorage.removeItem('sacred_city_filter'); localStorage.removeItem('sacred_religion_filter'); }} className="flex-1 py-3 border border-gray-300 rounded-lg text-gray-600 font-medium">Reset</button>
                    <button onClick={() => { setShowFilters(false); localStorage.setItem('sacred_city_filter', filterCity); localStorage.setItem('sacred_religion_filter', filterReligion); if(filterDistance) localStorage.setItem('sacred_distance_filter', filterDistance); else localStorage.removeItem('sacred_distance_filter'); fetchCandidates(session.user.id, profile.gender, profile) }} className="flex-1 py-3 bg-rose-600 text-white rounded-lg font-medium shadow-lg">Apply</button>
                </div>
            </div>
          </div>
        </div>
